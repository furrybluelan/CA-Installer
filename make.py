...
class Builder:
    ...
    CERT_NAME_JAVA = Path("CertName.java")
    CERT_NAME_CLASS = Path("CertName.class")
    CERT_NAME_DEX = Path("CertName.dex")
    MODULE_DEX = MODULE_DIR / "CertName.dex"
    ...
    def create_zip(self) -> bool:
        ...
        try:
            ...
            if self.OUTPUT_ZIP.exists():
                ...
                logger.info(f"Successfully created {self.OUTPUT_ZIP} ({size_mb:.2f} MB)")
                return True
            else:
